...
#define HIT_THRESH 6500

//3-bit RRIP counter
#define MAXRRIP 7
uint32_t rrip[LLC_SETS][LLC_WAYS];

uint64_t hawkeye_hit_counter;
uint64_t lru_hit_counter;
bool policy;
...
{
    cout << "Initialize LRU-eye replacement policy state" << endl;
	hawkeye_hit_counter = 10000;
	lru_hit_counter = 10000;
	policy = 0;
...
uint32_t lru_victim(const BLOCK *current_set)
{
  return std::distance(current_set, std::max_element(current_set, std::next(current_set, LLC_WAYS), lru_comparator<BLOCK, BLOCK>()));
}

uint32_t hawkeye_victim(uint32_t set)
{
...
uint32_t CACHE::find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t PC, uint64_t paddr, uint32_t type)
{
	...
	uint32_t hawkeye = hawkeye_victim(set);
	uint32_t lru = lru_victim(current_set);
...
void CACHE::update_replacement_state (uint32_t cpu, uint32_t set, uint32_t way, uint64_t paddr, uint64_t PC, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
	...
	if (policy && (hawkeye_hit_counter < HIT_THRESH))
	{	
		policy = 0;
		hawkeye_hit_counter = 10000;
	}

	else if (!policy && (lru_hit_counter < HIT_THRESH))
	{
		policy = 1;
		lru_hit_counter = 10000;
	}
	...
	auto begin = std::next(block.begin(), set * LLC_WAYS);
	auto end = std::next(begin, LLC_WAYS);
